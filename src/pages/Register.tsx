...
import { useAppDispatch, useWeb3Helper } from '../hooks/hooks';
...
const Register = () => {
  ...
  const { setUsersData, setAuthorization, setWalletAddress} = profileSlice.actions;
      ...
      try {
        ...
        .then(json => {
          if(json.token && json.type){
            dispatch(setAuthorization(json.type + ' ' + json.token));
            var authorization = json.type + ' ' + json.token;
            fetch('https://api.monopoly-dapp.com/users/profile/', { 
              headers: {
                  'accept': 'application/json',
